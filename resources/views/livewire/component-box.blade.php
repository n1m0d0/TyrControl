<div class="mt-2">
                    ...
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                        ...
                        <td class="px-6 py-4">
                            <ul>
                                ...
                                <li>
                                    <a wire:click='showDelete({{ $box->id }})'
    ...
    <flux:modal name="box-form" class="md:w-120">
        <div class="space-y-6">
            <div>
                @if ($activity == 'create')
                    <flux:heading size="lg">{{ __('Create box') }}</flux:heading>
                @else
                    <flux:heading size="lg">{{ __('Edit box') }}</flux:heading>
                @endif
            </div>

            <flux:input wire:model='form.name' label="{{ __('Name') }}"
                placeholder="{{ __('Example') }}: Box 0001" />

            <flux:input wire:model='form.code' label="{{ __('Code') }}"
                placeholder="{{ __('Example') }}: CB-0001" />
            ...
            <flux:select class="mt-2" wire:model="form.branch"
                wire:key="branch-select-{{ $activity }}-{{ $selectedBranch ?? 'none' }}">
            ...
            @if ($activity == 'edit' && $selectedBranch)
                @php $form->branch = $selectedBranch; @endphp
            @endif

            <flux:error name="form.branch" />
    ...
    <flux:modal name="box-delete" class="md:w-120">
        <div class="space-y-6">
            <div>
                <flux:heading size="lg">{{ __('Delete box') }}</flux:heading>
