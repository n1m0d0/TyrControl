<div class="mt-2">
                ...
                @foreach ($companies as $company)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                        <th scope="row"
                            ...
                            <img src="{{ Storage::url($company->logo) }}" alt="Current"
                        ...
                        <td class="px-6 py-4">
                            <ul>
                                <li>
                                    <a wire:click='showForm({{ $company->id }})'
                                ...
                                <li>
                                    <a wire:click='showDelete({{ $company->id }})'
    ...
    <flux:modal name="company-form" class="md:w-120">
        <div class="space-y-6">
            <div>
                @if ($activity == 'create')
                    <flux:heading size="lg">{{ __('Create company') }}</flux:heading>
                @else
                    <flux:heading size="lg">{{ __('Edit company') }}</flux:heading>
                @endif
            </div>
            ...
            <flux:input wire:model='form.name' label="{{ __('Name') }}"
                placeholder="{{ __('Example') }}: My company" />
    ...
    <flux:modal name="company-delete" class="md:w-120">
        <div class="space-y-6">
            <div>
